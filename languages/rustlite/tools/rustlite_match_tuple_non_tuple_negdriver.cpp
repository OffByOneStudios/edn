// Negative driver: match arm tuple pattern non-tuple target -> E1455
#include <iostream>
#include "edn/edn.hpp"
#include "rustlite/expand.hpp"
int main(){
    const char* src = R"((module :id "match_tuple_non_tuple_neg"
        (rfn :name "main" :ret i32 :params [] :body [
            (const %x i32 42)
            (tget %a i32 %x 0) (tget %b i32 %x 1)
            (tuple-pattern-meta %x 2)
    ...
    if(!saw){ std::cerr << "[match-tuple-non-tuple-neg] expected E1455 not found\n"; return 1; }
    std::cout << "[match-tuple-non-tuple-neg] ok\n"; return 0; }
