...
static void test_scalar_const_global(){
    ...
    assert(ok); if(m){ assert(m->getGlobalVariable("G") && m->getGlobalVariable("G")->isConstant()); }
}
...
static void test_array_const_global(){
    ...
    assert(r.success); if(m){ assert(m->getGlobalVariable("A") && m->getGlobalVariable("A")->isConstant()); } }
...
static void test_struct_const_global(){
    ...
    assert(r.success); if(m){ assert(m->getGlobalVariable("S") && m->getGlobalVariable("S")->isConstant()); } }
