...
Progress summary: Environment, Exceptions, and DataLayout usage are centralized; the structure-collection prepass is complete and wired, and globals emission moved into `ir/collect`. Build and tests are green on macOS.
...
Next steps:
- Extract `core_ops` (scalars, arith/bitwise, ptr math) and `memory` (addrs/deref/load/store) into `ir/` and swap call sites.
- Factor control flow (`if/while/for/switch/match`) into `ir/control`.
...
- [ ] core_ops.cpp: Scalars, arithmetic/bitwise, ptr math. (Inline in `src/edn.cpp`; to extract under `ir/`)
- [ ] memory.cpp: addrs/deref/load/store/globals/allocas; var slots. (Inline in `src/edn.cpp`; to extract)
- [ ] control.cpp: if/while/for/switch/match scaffolding and PHIs. (Inline in `src/edn.cpp`; to extract)
- [ ] sums.cpp: sum-new/sum-is/sum-get, match bindings; SumLayout helper. (Logic present but not extracted; DL helpers ready)
...
- [ ] di.cpp: Debug info (types, locals, struct members) guarded by EDN_ENABLE_DEBUG. (Currently inline; partial usage; to extract)
